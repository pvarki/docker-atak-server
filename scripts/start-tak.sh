...
if [ $1 = "messaging" ]; then
    ...
    java -jar -Xmx${CONFIG_MAX_HEAP}m -Dlogging.level.root=DEBUG -Dlogging.level.tak.server=TRACE ``-Dlogging.level.com.bbn.marti=TRACE -Dspring.profiles.active=config takserver.war
elif [ $1 = "api" ]; then
